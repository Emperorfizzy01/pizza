 ...
 var tokenId = typeof(app.config.sessionToken.id) == 'string' ? app.config.sessionToken.id : false;
    const button = document.getElementById("button3")
      button.addEventListener("click", async function() {
        const response = await fetch("/api/cart?emailAddress="+emailAddress, {
          method: 'GET',
          headers: {
        ...
        if(response.status == 200) {
          window.location = '/menus/cart'
        }
      })    
}

app.makeRequest = function(){
  const ids = [{id: 0, item: "Champagne"}, {id: 1, item: "Henessy"}, {id: 2, item: "MC dowell"}, {id: 3, item: "Fried rice"}, {id: 4, item: "Pizza"}, {id: 5, item: "Salad"}, {id: 6, item: "Turkey"}, {id: 7, item: "Rice"}]
  var tokensId = typeof(app.config.sessionToken.id) == 'string' ? app.config.sessionToken.id : false;
  ids.forEach((id) => {
    const button = document.getElementById("myButton"+id.id)
    button.addEventListener("click", async function() {
